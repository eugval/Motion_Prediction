...
from experiments.model import    Unet, UnetShallow, SpatialUnet, SpatialUnet2, SpatialUNetOnFeatures, UNetOnFeatures
...
def train_func(data_names, device):
    for data_name in data_names:
        ...
        ###### PARAMETERS #######
        descriptive_text = '''
        UnetOnFeatures, fixed mask feature bug
         '''


        #inputs, label and model params
        model = UNetOnFeatures
        if(debug):
            model_name = "model_test_1".format(data_name) # For test change here
        else:
            model_name = "UNetOnFeatures_MI_{}_3".format(data_name) # For test change here
